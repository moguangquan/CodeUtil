...
public class BackupFileUtil {
	private volatile static BackupFileUtil uniqueInstance = null;
	...
	private BackupFileUtil() throws IOException {
		...
		prop.load(BackupFileUtil.class.getClassLoader().getResourceAsStream(
				"backup.properties"));
		...
		try {
			...
			if (uniqueInstance == null){
				synchronized (BackupFileUtil.class) {
					if(uniqueInstance==null){
						uniqueInstance = new BackupFileUtil();	
					}
				}
			}
	...
	public List<BackupFile> listBackups() {
		File[] fs = new File(realPath + File.separator + backupFile)
		...
		for (File f : fs) {
			...
			bf.setFiletype(f.getName().substring(
	...
	public void backup(String name) {
		String bp = realPath + File.separator + backupFile + File.separator
		...
		try {
			...
			for (String f : backupFiles) {
				...
				String dest = bp + File.separator+ f;
				System.out.println("dest:"+dest);
				FileUtils.copyDirectory(new File(src), new File(dest));
			}
			// 3、完成打包tar和压缩gz
			TarAndGzipUtil tagu = TarAndGzipUtil.getInstance();
			tagu.tarFile(bp, realPath + File.separator + backupFile
					+ File.separator + new Date().getTime() + "_" + name
					+ ".tar");
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
			try {
				FileUtils.deleteDirectory(new File(bp));// 删除备份的文件夹，因为已经有gz压缩文件了
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	...
	/**
	 * 备份文件的恢复
	 * @param name 恢复的name就是整个文件的名称
	 */
	public void resume(String name) {
		String op = realPath + File.separator + backupFile + File.separator
				+ BACKUP_NAME;
		try {
			// 1、将文件进行解压缩
			String fp = realPath + File.separator + backupFile + File.separator
					+ name;
			TarAndGzipUtil tagu = TarAndGzipUtil.getInstance();
			...
			for (String f : backupFiles) {
				// 先删除原有的文件夹
				String src = op + File.separator+ f;
	...
	public void delete(String name) {
		String deletePath=realPath + backupFile+ File.separator + name;
